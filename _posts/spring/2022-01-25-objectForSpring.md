...
앞서 이야기 했던 것 처럼 모든 클래스들은 실행시에 스태틱 영역에 생성됩니다. 이때, 클래스 인스턴스 멤버변수에는 공간을 할당하지 않고, static 멤버변수에만 공간을 할당하여 모든 객체가 값을 공유하게 합니다. 그 후, 객체를 생성하면 객체는 T메모리의 힙 영역에 생성되며, 객체_참조_변수는 객체의 주소를 갖고 있게됩니다. 이는 정적 메서드와 인스턴스 메서드에도 마찬가지로 적용됩니다. <br><br>
